...
- **Thanks to improved training data:** stronger brain prediction models that can be (1) distilled down into (slightly weaker but) smaller and faster versions of themselves or (2) used to generate even more and better synthetic data via TTC.
...
We know we can generate images from text and other images. By extension, we can now generate beautiful, high quality videos from text and images. Increasingly, we are able to generate playable/explorable/interactive 3D scenes from text and images [^4][^7][^8][^9]. Along the parallel from Stability, if we can generate images from brain activity, we can (or will be able to) generate videos from brain activity, and then explorable/interactive 3D scenes from brain activity. Combine this from the brain state prediction results, it follows that we will be able to generate images, videos, and novel immersive experiences **from brain activity that has not yet occurred.** 

At some point a chicken and egg problem arises: is the next predicted brain state representative of the next actual brain state or is it one that, in a way, has been generated conditioned on the world that is being continuously generated (in response to previous brain states)? In this case, an interesting feedback loop is created. Are we predicting the next brain state or are we essentially generating the next brain state by generating the appropriate stimuli? 
...
- **Perfect Generation:** On the flip side, improvements in immersive technologies—like high-fidelity VR, direct neural stimulation, and adaptive multi-modal content generation—could allow us to craft experiences that align almost perfectly with any desired outcome.
...
As our abilities to predict and generate brain states advance, we face a fundamental question: how far ahead can we accurately forecast neural activity? If these models continue to scale—following the trajectory we've seen with language and world models—could we extend predictions from seconds to minutes, hours, or even days?
...
I ask again: when you get driving directions, do you follow the route that has been planned for you? It's likely that you do. But of course you could take another route or drive off the road altogether if your heart desired. But do you? This is a crude example. For better and worse, I do not think it will be this overt. In reality, you will not even be aware that you are being steered towards coffee. You simply became aware that you wanted a coffee (well after your AI had already become bored of this eventuality) and magically one was ready for you. Success! 

*Or maybe on that particular day Starbucks paid OpenAI enough in 'advertising' to subtly nudge people like you toward this seemingly innocuous craving...*
...
In this context, I ask again: are my experiences, interactions, and brain states the ones being optimized for, are yours the ones being optimized for, or are we all part of a collective that is being optimized? *Or (exhaustingly) are all our actions and experiences now able to be imperceivably steered to optimize returns for the highest bidder(s)?*
...
Or maybe it will not be so linear. Maybe we will be able to set preferences that that can be used to shape our experiences. Take weight loss for example. Did your model predict that you're about to crave that extra slice of pizza? Maybe the generative model can pivot, generating new content to proactively distract you, preemptively curbing that craving before it ever existed. Or did your model predict that you're likely to be sad tonight? Perhaps your AI assistant can come in with positive words of encouragement, recall a recent memory of a time you were happy, or conjure up [a scene of utmost cuteness](https://www.tiktok.com/@openai/video/7336713538532134186){:target="_blank"}.

*But wait, was it my own free will to set these preferences in the first place or was I expertly nudged (read: guided, manipulated) into setting them?*
...
- Process and reshape sensitive or traumatic memories in a controlled environment.

These aren't just science fiction concepts anymore. The technical foundation is already being laid. **What will you choose to remember?**
...
The question transforms from **"What will you choose to remember?"** to **"What reality do you want to experience?"**
